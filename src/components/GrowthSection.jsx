...
export default function GrowthSection() {
  ...
  return (
    // UPDATED: 'px-2' on mobile makes the background stretch much closer to the edges
    <section ref={sectionRef} className="bg-background w-full text-brown-900 py-8 px-2 sm:py-10 sm:px-12">
      
      {/* UPDATED: 'p-4' on mobile reduces internal spacing, making the content feel wider */}
      <div ref={containerRef} className="max-w-[1400px] mx-auto bg-bg-growth rounded-3xl p-4 md:p-10 shadow-lg flex flex-col gap-8">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="pl-2">
            <h2 className="text-3xl sm:text-4xl md:text-5xl text-text-primary font-primary font-bold text-dark">
              The path to <span className="text-text-secondary font-serifa font-normal">Growth</span>
            </h2>
            <p className="text-base md:text-lg text-text-primary font-primary mt-2 text-dark">“36 weeks, three phases, global reach”</p>
          </div>
        ...
        {/* Phases Grid: Reduced padding (p-5) on mobile */}
        <div className="bg-bg-card rounded-[30px] p-5 md:p-12 text-text-card">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8 relative">
            {phases.map((phase, index) => (
              <div key={phase.id} className="relative flex flex-col gap-6 group">
                
                {/* Phase Header */}
                <div className="border-b border-white/10 pb-4 md:min-h-[110px] flex flex-col justify-start">
                  <h3 className="text-xl md:text-2xl font-primary">
                    <span className="opacity-50 text-xs md:text-sm uppercase tracking-widest block mb-1">Phase 0{phase.id}</span>
                    <span className="font-bold text-2xl md:text-3xl text-bg-growth">{phase.title}</span>
                  </h3>
                </div>
                ...
                <div className="flex flex-col gap-4 md:gap-2">
                  {phase.steps.map((step, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      {/* 1. DESKTOP ONLY: Gradient Number */}
                      <GradientNumber 
                        number={i + 1} 
                        theme={theme} 
                        variant="fill" 
                        className="hidden md:flex w-14 h-14 flex-shrink-0" 
                      />

                      {/* 2. MOBILE ONLY: Simple Pointer (Dot) */}
                      <div className="md:hidden flex-shrink-0 mt-2.5 w-1.5 h-1.5 rounded-full bg-[#F47A36]"></div>
