...
const VentureServices = () => {
  ...
  const headingDrawLineRef = useRef(null);
  const headingCircleRef = useRef(null);

  useGSAP(() => {
    if (!pinRef.current || !mainRef.current) return;

    const headingPath = headingDrawLineRef.current;
    const headingCircle = headingCircleRef.current;

    if (headingPath && headingCircle) {
      gsap.set(headingPath, { drawSVG: "0% 0%" });
      gsap.set(headingCircle, { scale: 0, transformOrigin: "center center" });
      ...
      .to(headingPath, { drawSVG: "0% 100%", ease: "power1.inOut" })
      .to(headingCircle, { scale: 1, ease: "back.out(1.7)" }, "-=0.1");
    }
    ...
    mainTimeline
      .to('#mentorship-content', { opacity: 0 }, "first fadeout")
      .to('#investment-content', { opacity: 1 }, "first fadin")
      .to('.morph-path', { morphSVG: (i) => ventureData.svg.paths.investment[i], ease: 'power1.inOut' }, "first fadeout")
      .to('.morph-dot', { attr: (i) => ({...ventureData.svg.dots.investment[i]}), ease: 'power1.inOut' }, "first fadeout")
      .to('#investment-content', { opacity: 0 }, "second fadeout")
      .to('#connects-content', { opacity: 1 }, "second fadein")
      .to('.morph-path', { morphSVG: (i) => ventureData.svg.paths.connects[i], ease: 'power1.inOut' }, "second fadeout")
      .to('.morph-dot', { attr: (i) => ({...ventureData.svg.dots.connects[i]}), ease: 'power1.inOut' }, "second fadeout");
    ...
    <div ref={mainRef} className="h-[300vh] bg-black text-[#FFC7A8]">
      <div ref={pinRef} className="h-screen w-screen flex flex-col">
        <div className="w-full text-center py-12 relative flex flex-col items-center justify-center">
          <div className="absolute top-20 left-1/6">
...
<line x1="1162" y1="197.779" x2="1" y2="197.779" stroke="#F47A36" stroke-width="0.855262"/>
<line y1="-0.427631" x2="194.819" y2="-0.427631" transform="matrix(0.00536359 -0.999986 0.999984 0.00573948 1 197.488)" stroke="#F47A36" stroke-width="0.855262"/>
<line x1="1161.57" y1="198" x2="1161.57" y2="149" stroke="#F47A36" stroke-width="0.855262"/>
</svg>
          ...
          <div className="relative z-10 max-w-[900px] w-full flex justify-center items-center">
            <h1 className='text-5xl md:text-6xl font-primary leading-tight'>
              THREE PILLARS,<br />
              ONE BRIDGE TO <br />
              <span 
                style={{
                ...
                className='text-[#F47A36] text-7xl md:text-[82px] leading-none tracking-tight'
              >
                success
              </span>
            </h1>
          ...
          <div className="absolute top-full -translate-y-1/2 mt-4 w-[330px] h-[6px] z-0">
            {/* <svg width="330" height="6" viewBox="0 0 330 6" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path ref={headingDrawLineRef} d="M0 3H327" stroke="#F47A36" strokeWidth="1"/>
              <circle ref={headingCircleRef} cx="327" cy="3" r="3" fill="#F47A36" />
            </svg> */}
          </div>
        </div>

        <div className="flex-grow flex items-center justify-center w-full px-4 pb-8 overflow-hidden">
          <div 
            className="content-wrapper relative grid grid-cols-10 max-w-[1100px] w-full h-full max-h-[670px] border border-[#F47A36] rounded-[2rem] bg-black overflow-hidden md:grid-cols-10"
          >
            <div className="col-span-10 md:col-span-6 relative flex items-center justify-center p-6 md:p-12 border-b md:border-b-0 md:border-r border-[#F47A36]">
              {ventureData.content.map((item, index) => (
                <div key={item.id} id={`${item.id}-content`} className={`absolute w-full h-full flex flex-col gap-8 md:gap-16 items-center justify-center p-6 md:p-12 text-center ${index === 0 ? 'opacity-100' : 'opacity-0'}`}>
                   <div className="flex flex-col items-center gap-0">
                    <h1 className="text-3xl md:text-4xl font-primary font-normal leading-none">{item.title}</h1>
                    <h2 className="text-base md:text-[18px] font-secondary font-normal text-[#F47A36]">{item.subtitle}</h2>
                  </div>
                  <p className="text-sm md:text-[18px] font-secondary leading-tight md:leading-[22px]">{item.description}</p>
                </div>
              ))}
            </div>
            
            <div className="hidden md:col-span-4 md:flex flex-col justify-center items-center gap-6 p-4">
              {/* --- This is the new wrapper div for the background --- */}
              <div
                className="w-[400px] h-[400px] rounded-2xl flex items-center justify-center"
                style={{
