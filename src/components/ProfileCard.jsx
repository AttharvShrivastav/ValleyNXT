...
const LinkedInIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="currentColor" className="text-icon opacity-80 group-hover:opacity-100">
        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
    </svg>
);
    ...
    return (
        <div ref={card} className={`relative flex-shrink-0 w-[280px] h-[360px] bg-profilecard-bg rounded-[35px] overflow-hidden cursor-pointer ${className}`}>
            ...
            <div className="relative z-10 h-full">
                ...
                {/* ✅ CHANGE: Added padding-top to the text container to prevent overlap with the top-right icon */}
                <div className="absolute text-left bottom-6 left-6 right-6 pr-14 pt-12">
                    <h3 className="text-xl font-bold text-white [text-shadow:1px_1px_3px_rgba(0,0,0,0.5)]">
                        {person.name}
                    </h3>
                    <span className="mt-1 block text-base text-white leading-tight break-words [text-shadow:1px_1px_3px_rgba(0,0,0,0.5)]">
            ...
            <a
                ...
                rel="noopener noreferrer"
                onClick={(e) => e.stopPropagation()}
                // ✅ FIX: Changed `bottom-6` to `top-6` to move the icon to the top right corner.
                className="group absolute top-6 right-6 z-30 transition-transform duration-300 hover:scale-110"
                aria-label={`${person.name}'s LinkedIn Profile`}
            >
                <LinkedInIcon />
            </a>
