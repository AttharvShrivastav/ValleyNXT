...
const VnvpediaSection = () => {
    ...
    useGSAP(() => {
        ...
        // 2. Added '.cta-container' to the entrance animation
    ...
    return (
        <div ref={sectionContainerRef} className="relative bg-black min-h-screen flex flex-col items-center justify-center py-24 px-4 font-primary">
            ...
            <div className="relative w-full max-w-5xl flex flex-col items-center mb-24">
                <div className="relative z-20 flex items-center justify-center w-full">
                    <div className="heading-frame for-svg absolute -translate-y-6">
                    ...
                    <h2 className="heading-text text-white text-6xl md:text-6xl font-primary tracking-wider text-center">
                        EXPLORE <span className="text-[#F47A36] text-8xl font-serifa">vnvpedia</span>
                    </h2>
                </div>
            </div>

            <div className="relative w-full max-w-[76rem] h-[30rem]">
                <div className="back-plate absolute inset-0 w-full h-[26rem] top-1/2 -translate-y-1/2 border-2 border-[#F47A36] rounded-[35px] z-10"></div>
                <button
                    onClick={() => handleNavigation('prev')}
                    className="nav-button absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 z-30 p-4 rounded-full bg-[#f47a36] text-black hover:scale-110 transition-transform duration-300 focus:outline-none"
                ...
                <div className="main-card absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[68rem] h-[30rem] border-2 border-[#f47a36] bg-[#FAD9C5] rounded-[35px] text-center shadow-lg flex items-center justify-center p-8 md:p-12 z-20 overflow-hidden">
                    <div className="relative z-10 w-full max-w-2xl h-full flex items-center justify-center">
                        <div ref={el => contentRefs.current[0] = el} className="absolute w-full">
                            <h3 className="text-3xl md:text-4xl font-secondary font-semibold mb-6 text-[#d98d62]">{vnvpediaArticles[0].title}</h3>
                            <p className="text-lg md:text-xl font-secondary leading-relaxed text-[#2b1107]">{vnvpediaArticles[0].content}</p>
                        </div>
                        <div ref={el => contentRefs.current[1] = el} className="absolute w-full">
                             <h3 className="text-3xl md:text-4xl font-secondary font-semibold mb-6 text-[#d98d62]"></h3>
                             <p className="text-lg md:text-xl font-secondary leading-relaxed text-[#2b1107]"></p>
                        </div>
                    </div>
                </div>
                <button
                    onClick={() => handleNavigation('next')}
                    className="nav-button absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 z-30 p-4 rounded-full bg-[#f47a36] text-black hover:scale-110 transition-transform duration-300 focus:outline-none"
            ...
            {/* 3. Added the new button in a container */}
